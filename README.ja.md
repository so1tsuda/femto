...
| ファイル操作 | `C-x C-f`（開く）、`C-x C-s`（保存）、`C-x C-w`（別名保存）、`C-x C-c`（終了）、`C-x k`（バッファ kill） |
...
- 🖱️ **ドラッグ&ドロップ** — ファイルをエディタウィンドウにドラッグして開く
- 🗂️ **複数バッファ** — `C-x b` で開いているファイルを切り替え
